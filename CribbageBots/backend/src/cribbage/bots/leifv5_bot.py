"""
LeifV5Bot — Optimized state-tracking bot with tuned positional play and tie-broken Hail Mary mode.
"""
...
class LeifV5Bot(LeifV3Bot):
    ...
    def _discard_v5(self, hand: List[Card], is_dealer: bool) -> Tuple[Card, Card]:
        ...
        # Tuned thresholds and weights (identical to V4.5)
        if delta > 10:
            opp_crib_w = 1.2
            our_crib_w = 0.9
        elif delta < -10:
        ...
        # Precompute unique ranks in unseen
        unseen_ranks = set(c.numeric_rank for c in unseen)
        rank_to_value = {}
        for c in unseen:
            rank_to_value[c.numeric_rank] = c.value
        ...
        for keep_idx in combinations(range(n), 4):
            ...
            # 1. Calculate non-suit scores for each unique rank in unseen
            non_suit_score = {}
            for cn in unseen_ranks:
                cv = rank_to_value[cn]
                score = keep_15s
                if cv <= 15:
                    score += dp[15 - cv] * 2
                ...
                non_suit_score[cn] = score

            # 2. Evaluate all 46 unseen cards using precomputed non-suit scores + suit additions
            total = 0
            max_score = 0
            for c in unseen:
                cn = c.numeric_rank
                ...
                score = non_suit_score[cn]
                if keep_4flush:
                    score += 5 if cs == keep_flush_suit else 4
                if jack_suits:
            ...
            if hail_mary:
                # Primary key: max possible score
                # Secondary key: total expected value (tie-breaker)
