...
class GameEngine:
    ...
    def play_game(self) -> Tuple[str, List[dict]]:
        ...
        while not self.winner:
            try:
                ...
                # The offending player forfeits
                offender = str(e).split(":")[0] # Hacky way to pass who forfeited in message
                self.winner = p1_id if offender == p2_id else p2_id
                self.log_event("forfeit", offender, f"{offender} forfeited due to illegal move: {e}")
                break
        ...
        if self.state.scores[loser_id] <= 90:
            self.skunk = True
    ...
    def play_round(self):
        ...
        for pid in [non_dealer, dealer]:
            ...
            # Validate discards
            if len(discards) != 2 or discards[0] not in hands[pid] or discards[1] not in hands[pid] or discards[0] == discards[1]:
                raise IllegalMoveError(f"{pid}:Invalid discard {discards}")
    ...
    def run_pegging_phase(self, hands: Dict[str, List[Card]], non_dealer: str, dealer: str):
        ...
        while hands[non_dealer] or hands[dealer]:
            ...
            if len(passed) == 2:
                # Both passed, reset count
                self.award_points(last_to_play, 1, "Go (Last Card)")
                if self.winner: return
                self.state.current_count = 0
                self.state.peg_history = []
                passed.clear()
                # The player who DID NOT play last gets to start the new sequence
                current_player = non_dealer if last_to_play == dealer else dealer
                
                # Check if current player actually has cards, if not switch back
            ...
            if played_card is None:
                if legal_moves:
                    raise IllegalMoveError(f"{current_player}:Passed when legal moves exist")
                passed.add(current_player)
                self.log_event("peg_go", current_player, f"{current_player} says Go.")
            else:
                if played_card not in legal_moves:
                    raise IllegalMoveError(f"{current_player}:Played illegal card {played_card}")
                ...
                if self.state.current_count == 31:
                    ...
                    self.state.peg_history = []
                    passed.clear()
                    # After 31, the OTHER player starts
                    current_player, other_player = other_player, current_player
                else:
                    current_player, other_player = other_player, current_player

        # Award last card if loop ends and we didn't just hit 31
